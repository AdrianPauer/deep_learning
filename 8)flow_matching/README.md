...
In first part we implement U-net architecture for vector filed porediction. In part we add self-attention over the image pixels features.
...
Convolutional layers process information locally through their receptive fields, while self-attention allows every spatial position in the feature map to directly attend to every other position.
...
meaning that each image location attends to other locations within the same feature map.

After attention, the sequence is reshaped back to the original image representation:
...
and combined with the original features using a residual connection:

[
\text{output}=X+\text{Attention}(X)
]

Self-attention is applied mainly at lower spatial resolutions because the attention matrix has quadratic complexity:

[
O((H\cdot W)^2)
]

Lower resolutions provide global context while keeping the computational cost manageable.
